...
class WorkflowRunQueueJob(models.Model):
    ...
    def _sync_queue_job_link(self):
        missing_links = self.filtered(
            lambda run: run.queue_job_uuid and not run.queue_job_id
        )
        ...
        for run in missing_links:
            queue_job = queue_job_model.search(
                [("uuid", "=", run.queue_job_uuid)], limit=1
            )
            if queue_job:
                ...
                run.write(
                    {
                        ...
                        "error_message": _(
                            "Queued workflow execution failed before completion."
                        ),
                    }
                )
            raise
