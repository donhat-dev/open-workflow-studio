...
class QueueJob(models.Model):
    ...
    def _sync_linked_workflow_runs(self):
        for job in self.filtered('workflow_run_id'):
            ...
            elif job.state == 'cancelled' and run.status not in ('completed', 'failed', 'cancelled'):
                updates['status'] = 'cancelled'
                if not run.completed_at:
                    updates['completed_at'] = job.date_cancelled or fields.Datetime.now()
    ...
    def write(self, vals):
        ...
        if set(vals) & {'state', 'date_started', 'date_done', 'date_cancelled', 'exc_message', 'result'}:
