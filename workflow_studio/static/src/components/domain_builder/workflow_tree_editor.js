...
/**
 * WorkflowTreeEditor — clone of Odoo's TreeEditor with one key diff:
 * getValueEditorInfo() delegates to getWorkflowValueEditorInfo() which
 * wraps every native value editor with an ExpressionValueEditor toggle.
 *
 * All other logic (tree manipulation, operator/path editors, recursive
 * rendering) is identical to the original.
 *
 * CSS classes kept identical to Odoo's for styling parity.
 */
...
import { ExpressionValueEditor } from "./expression_value_editor";
import { getWorkflowValueEditorInfo } from "./domain_builder_utils";
...
export class WorkflowTreeEditor extends Component {
    static template = "workflow_studio.WorkflowTreeEditor";
    static components = {
        Dropdown,
        DropdownItem,
        ModelFieldSelector,
    ...
    // ──── KEY DIFF from Odoo's TreeEditor ────
    // Uses getWorkflowValueEditorInfo which wraps native editors with
    // ExpressionValueEditor (literal↔expression toggle).
    // ExpressionValueEditor is passed as an option to break the circular
    // dependency between domain_builder_utils ↔ expression_value_editor.
    getValueEditorInfo(node) {
        const fieldDef = this.getFieldDef(node.path);
        return getWorkflowValueEditorInfo(fieldDef, node.operator, {
            ExpressionValueEditor,
            inputContext: this.props.inputContext,
    });
    }
