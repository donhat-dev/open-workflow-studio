...
class TestCustomNodeRuntime(common.TransactionCase):
    ...
    def _create_custom_type(self, node_type=None, code='result = _json', group=None, active=True):
        vals = {
    ...
    def test_is_custom_computed_and_available_types_flag(self):
        ...
        builtin_type = self._create_builtin_type(node_type=self._new_builtin_key('flag'))
        ...
        by_key = {
            item['node_type']: item
            for item in self.WorkflowType.get_available_types()
        }
    ...
    def test_custom_runtime_mapping_active_custom_only(self):
        ...
        self.assertEqual(mapping[active_custom.node_type]['group_id'], self.runtime_group.id)
    ...
    def test_executor_executes_custom_runtime_code(self):
        ...
        executor, output = self._execute_snapshot(snapshot, input_data={'hello': 'world'})
    ...
    def test_executor_fails_for_unknown_custom_type(self):
        ...
        workflow = self._create_workflow(run_as_user=self.allowed_user)
        snapshot = self._build_snapshot(custom_node_type=unknown_node_type, workflow=workflow)
