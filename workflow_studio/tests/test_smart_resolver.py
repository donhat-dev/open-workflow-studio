...
Tests the unified 3-tier expression resolution:
    1. Single {{ expr }} → type-preserving eval
    2. Mixed text {{ a }} {{ b }} → string interpolation
    3. No {{ }} → literal passthrough
"""
...
class TestSmartExpressionResolver(common.TransactionCase):
    ...
    def test_mixed_multiple_templates(self):
        result = self.resolver.resolve('{{ _json.name }} ({{ _json.id }})', self.ctx)
        self.assertEqual(result, 'Alice (42)')

    def test_mixed_url_pattern(self):
        result = self.resolver.resolve(
            'https://api.example.com/users/{{ _json.id }}/profile', self.ctx
        )
        self.assertEqual(result, 'https://api.example.com/users/42/profile')
    ...
    def test_mixed_invalid_expr_returns_empty_replacement(self):
    ...
    def test_resolve_domain_leaf_with_expr(self):
        """Per-leaf {{ expr }} in already-resolved domain list."""
        domain = [('id', '=', '{{ _json.id }}')]
        result = self.resolver.resolve_domain(domain, self.ctx)
        self.assertEqual(result, [('id', '=', 42)])

    def test_resolve_domain_leaf_partial_rejected(self):
        """Partial templates in leaf values are rejected."""
        domain = [('id', 'in', '[{{ _json.id }}]')]
        with self.assertRaises(ValueError):
            self.resolver.resolve_domain(domain, self.ctx)
    ...
    # ------------------------------------------------------------------
    # ExpressionEvaluator backward compatibility
    # ------------------------------------------------------------------

    def test_legacy_evaluator_delegates(self):
        from ..models.runners.base import ExpressionEvaluator
        result = ExpressionEvaluator.evaluate('{{ _json.id }}', self.ctx)
        # Legacy always returns string
        self.assertEqual(result, '42')
        self.assertIsInstance(result, str)

    def test_legacy_evaluator_mixed(self):
        from ..models.runners.base import ExpressionEvaluator
        result = ExpressionEvaluator.evaluate('id={{ _json.id }}', self.ctx)
        self.assertEqual(result, 'id=42')
