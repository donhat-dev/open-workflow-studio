...
class WorkflowRun(models.Model):
    ...
    name = fields.Char(
        ...
        required=True,
        readonly=True,
        default=lambda self: self.env["ir.sequence"].next_by_code("workflow.run")
        or _("New"),
    ...
    started_at = fields.Datetime(
        string="Started At", index=True, help="Execution start time"
    )
    completed_at = fields.Datetime(
        string="Completed At", help="Execution completion time"
    )
    ...
    executed_version = fields.Integer(
        string="Executed Version", help="Workflow version that was executed"
    )
    ...
    error_message = fields.Text(
        string="Error Message", help="Error message if execution failed"
    )
    error_node_id = fields.Char(
        string="Error Node ID", help="Node where error occurred"
    )

    # === Execution Stats ===
    node_count_executed = fields.Integer(
        string="Nodes Executed", default=0, help="Number of nodes that were executed"
    )
...
class WorkflowRunNode(models.Model):
    ...
    # === Node Reference ===
    node_id = fields.Char(
        string="Node ID", required=True, index=True, help="Node ID from snapshot"
    )
    node_type = fields.Char(string="Node Type", help="Type of node executed")
    node_label = fields.Char(
        string="Node Label", help="Label of node at execution time"
    )

    # === Execution Order ===
    sequence = fields.Integer(
        string="Execution Order", default=0, help="Order in which node was executed"
    )
    iteration = fields.Integer(
        string="Iteration", default=0, help="Loop iteration number (0 = first/only run)"
    )
    ...
    # === Timing ===
    started_at = fields.Datetime(string="Started At", help="Node execution start")
    completed_at = fields.Datetime(
        string="Completed At", help="Node execution completion"
    )
    ...
    # === Data ===
    input_data = fields.Json(
        string="Input Data", default=lambda self: {}, help="Data received by node"
    )
    output_data = fields.Json(
        string="Output Data", default=lambda self: {}, help="Data produced by node"
    )
    output_socket = fields.Char(
        string="Output Socket", help="Which output socket was used"
    )

    # === Error ===
    error_message = fields.Text(string="Error Message", help="Error if node failed")
