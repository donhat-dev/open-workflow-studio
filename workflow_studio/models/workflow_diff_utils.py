    ...
    for k in new_map:
        if k in old_map and new_map[k] != old_map[k]:
            ...
            if changes:
                modified.append({
                    'id': k,
                    'label': new_map[k].get(label_key) or new_map[k].get('type') or k,
                    'new': new_map[k],
                    'old': old_map[k],
                    'changes': changes
                })
...
def _render_comparison_html(comparison):
    ...
    has_changes = (
        nodes['added'] or nodes['removed'] or nodes['modified'] or
        conns['added'] or conns['removed'] or conns['modified'] or
        meta['changed']
    )
