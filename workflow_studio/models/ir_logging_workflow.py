...
class IrWorkflowLogging(models.Model):
    ...
    workflow_run_id = fields.Integer(
        string="Workflow Run ID", index=True, help="ID of workflow.run record"
    )
    workflow_node_id = fields.Integer(
        string="Workflow Node ID", index=True, help="ID of workflow.node record"
    )
    workflow_id = fields.Integer(
        string="Workflow ID", index=True, help="ID of ir.workflow record"
    )
    ...
    def _compute_workflow_run(self):
        for record in self:
            if record.workflow_run_id:
                record.workflow_run = self.env["workflow.run"].browse(
    ...
    def _compute_workflow_node(self):
        for record in self:
            if record.workflow_node_id:
                record.workflow_node = self.env["workflow.node"].browse(
