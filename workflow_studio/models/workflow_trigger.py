...
class WorkflowTrigger(models.Model):
    ...
    _sql_constraints = [
        ...
        ("webhook_uuid_uniq", "UNIQUE(webhook_uuid)", "Webhook UUID must be unique."),
        (
            "webhook_test_uuid_uniq",
            "UNIQUE(webhook_test_uuid)",
    ...
    def _compute_webhook_url(self):
        ...
        for rec in self:
            if rec.webhook_uuid:
                rec.webhook_url = (
                    f"{base_url}/workflow_studio/webhook/{rec.webhook_uuid}"
                )
            else:
    ...
    def _compute_webhook_test_url(self):
        ...
        for rec in self:
            if rec.webhook_test_uuid:
                rec.webhook_test_url = (
                    f"{base_url}/workflow_studio/webhook-test/{rec.webhook_test_uuid}"
                )
            else:
    ...
    def _activate_schedule(self):
        ...
        cron_vals = {
            ...
            "user_id": self.workflow_id.run_as_user_id.id
            or self.env.ref("base.user_root").id,
        }
    ...
    def get_panel_state(self):
        ...
        return {
            ...
            "webhook_last_test_triggered": self.webhook_last_test_triggered.isoformat()
            if self.webhook_last_test_triggered
            else False,
