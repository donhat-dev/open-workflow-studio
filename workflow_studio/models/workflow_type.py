...
class WorkflowType(models.Model):
    ...
    @staticmethod
    def _infer_runtime_backend(node_type, runtime_backend=None, runtime_code=None, callable_key=None):
    ...
    def _check_custom_node_contract(self):
        ...
        for record in self:
            ...
            if runtime_backend in ('python_code', 'python_callable') and not record.group_id:
                raise ValidationError(_(
    ...
    def write(self, vals):
        ...
        if needs_runtime_recompute:
            for record in self:
                effective_node_type = (vals.get('node_type') or record.node_type or '').strip()
                vals.setdefault(
                    'runtime_backend',
                    self._infer_runtime_backend(
                        node_type=effective_node_type,
                        runtime_backend=vals.get('runtime_backend') or record.runtime_backend,
                        runtime_code=vals.get('code') if 'code' in vals else record.code,
                        callable_key=vals.get('callable_key') if 'callable_key' in vals else record.callable_key,
                    ),
                )

        if set(vals) & {'node_type', 'output_schema', 'active', 'code', 'group_id', 'runtime_backend', 'callable_key'}:
            self.env.registry.clear_cache()
    ...
    def _get_custom_runtime_mapping(self):
        ...
        """
        self.flush_model(['node_type', 'code', 'group_id', 'active', 'is_custom', 'runtime_backend', 'callable_key'])
        self.env.cr.execute(
        ...
        mapping = {}
        for node_type_key, runtime_backend, code, callable_key, group_id in self.env.cr.fetchall():
    ...
    def _register_hook(self):
        ...
        try:
            ...
            _logger.exception("Failed to synchronize decorated workflow nodes during registry hook.")
        return result
